...
# This code is copied from weaverba137/pydl. See https://github.com/weaverba137/pydl
...
class yanny(OrderedDict):
    ...
    def get_token(string):
        ...
        """
        if string[0] == '"':
            (word, remainder) = re.search(r'^"([^"]*)"\s*(.*)', string).groups()
        elif string[0] == "{":
            (word, remainder) = re.search(r"^\{\s*([^}]*)\s*\}\s*(.*)", string).groups()
        else:
            try:
    ...
    def dtype_to_struct(dt, structname="mystruct", enums=None):
        ...
        return {
            structname.upper(): list(dt.names),
            "enum": returnenums,
            "struct": ["\n".join(lines)],
        }
    ...
    def type(self, structure, variable):
        ...
        try:
            ...
            defl = [x for x in self._symbols["struct"] if x.find(structure.lower()) > 0]
            defu = [x for x in self._symbols["struct"] if x.find(structure.upper()) > 0]
        ...
        *e.g.*: ``char[5][20]``.
        ...
        try:
            ...
            if (character_array.search(typ) is not None) or (
                typ.find("char") < 0 and (typ.find("[") >= 0 or typ.find("<") >= 0)
            ):
                cache[variable] = True
            else:
    ...
    def char_length(self, structure, variable):
        ...
        *e.g.* ``char[5][20]`` is an array of 5 strings of length 20.
        ...
        try:
            ...
            if self.isarray(structure, variable):
                return max(
                    [max([len(x) for x in r]) for r in self[structure][variable]]
                )
            else:
    ...
    def dtype(self, structure):
        ...
        """
        dt = list()
        dtmap = {"short": "i2", "int": "i4", "long": "i8", "float": "f", "double": "d"}
            ...
            for k in range(self.size(sym)):
                ...
                for col in columns:
                    if self.isarray(sym, col):
                        datum = (
                            "{"
                            + " ".join([self.protect(x) for x in self[sym][col][k]])
                            + "}"
                        )
                    else:
    ...
    def append(self, datatable):
        ...
        if len(self.filename) == 0:
            raise ValueError(
                "No filename is set for this object. "
        ...
        if len(contents) > 0:
            ...
            if os.access(self.filename, os.W_OK):
                ...
                raise PydlutilsException(
                    self.filename + " does not exist, aborting append!"
                )
        else:
    ...
    def _parse(self):
        ...
        self._symbols["struct"] = re.findall(
            r"typedef\s+struct\s*\{[^}]+\}\s*\w+\s*;", lines
        )
        self._symbols["enum"] = re.findall(
            r"typedef\s+enum\s*\{[^}]+\}\s*\w+\s*;", lines
        )
...
def write_ndarray_to_yanny(
    ...
    for k in range(len(datatables)):
        struct = par.dtype_to_struct(
            datatables[k].dtype, structname=structnames[k], enums=enums
        )
